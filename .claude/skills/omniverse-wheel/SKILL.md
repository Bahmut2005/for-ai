...
Spin the wheel and display a cinematic random media recommendation from the local database.
...
- If response is `200` → server is up, continue.
- If not → start the dev server in the background:
  ```bash
  ...
  for i in $(seq 1 15); do
    code=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/api/presets)
    [ "$code" = "200" ] && break
    sleep 1
  done
  ```
...
- You can confirm the app is running at any time with `/omniverse-wheel` — if the server is already up it returns instantly.
